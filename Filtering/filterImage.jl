...
include("../SESOP/src/SESOP_computed_B.jl")
...
nr=256;nc=256   # taille
blur = 5        # degré de brouillage
noise = 0.3      # niveau de bruit
img, Ac, Ar, X = test_image("lena_gray_256", noise=noise, blur = blur)
# img, Ac, Ar, X = square_circle(nr,nc, noise=noise, blur = blur)
#################################
...
let Aop=Aop, Vimg=Vimg, m2=m2
    for i in 50:50
        ...
        name = "Results/testSESOP_no_pcc_" * "$i.png"
        ...
        m1 = Quad_Op(Aop, Vimg, zeros(length(Vimg)))
        reset!(m1)
        reset!(m2)
        ...
        stp, iter, x, fx = SESOP_pastiche_pcc(nlp;stp=stp);
        @show getStatus(stp)
        @show stp.meta.nb_of_stop
        @show sum_counters(stp.pb)
        ...
        XsU = reshape(stp.current_state.x,nr,nc)
        Uimg = map(clamp01nan, XsU)
        name = "Results/testSESOP_pcc_" * "$i.png"
        ImageFiles && save(name, Uimg)
        ImageDisplay && imshow(Uimg, name = "SESOP without bounds")
        sleep(0.001)
