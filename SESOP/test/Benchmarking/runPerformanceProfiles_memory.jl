...
#(:SESOP_g0, :SESOP_0),
#, (:SESOP_g5, :SESOP_5)
for (fn, fnsimple) ∈ [(:SESOP_m1, :SESOP_1), (:SESOP_m2, :SESOP_2), (:SESOP_m3, :SESOP_3), (:SESOP_m0, :SESOP_4)]
    ...
    @eval begin
        ...
        function $fnsimple(nlp)
            ...
            rename!(df, :n => "SESOP_$((config))_$(nlp.meta.name)_$solvn")
            global solvn += 1
            ...
            open("test/Benchmarking/memory/memory_$(config).txt","a") do io
            ...
            open("test/Benchmarking/memory/memory_normes_$(config).txt","a") do io
                println(io, "$(nlp.meta.name), ", normes)
            end

            open("test/Benchmarking/memory/memory_objectives_$(config).txt","a") do io
...
open("test/Benchmarking/memory.txt","a") do io
    println(io, "_____________________________________________\n")
    println(io, df)
...
file = File{format"JLD2"}("Stats_memory.jld2")
