...
probnames = pbnames#[1:2]  # for testing quickly on a subset
...
for (fn, fnsimple) ∈ [(:SESOP_p0, :SESOP_0), (:SESOP_p1, :SESOP_1), (:SESOP_p2, :SESOP_2), (:SESOP_p3, :SESOP_3)]#, (:SESOP_p4, :SESOP_4), (:SESOP_p5, :SESOP_5)]
    ...
    @eval begin
        ...
        function $fnsimple(nlp)
            ...
            rename!(df, :n => "SESOP_$(pres)_$(nlp.meta.name)_$solvn")
            global solvn += 1
            ...
            open("test/Benchmarking/pres/pres_$(pres).txt","a") do io
                println(io, "_____________________________________________")
                println(io,"NLP = $(nlp.meta.name) | N = $(nlp.meta.nvar) | pres : [$(pres)]")
                println(io, d)
                println(io, "_____________________________________________\n")
            end

            open("test/Benchmarking/pres/pres_normes$(pres).txt","a") do io
                println(io, "$(nlp.meta.name), ", normes)
            end

            open("test/Benchmarking/confi/pres_objectives$(pres).txt","a") do io
...
open("test/Benchmarking/precision.txt","a") do io
    println(io, "_____________________________________________\n")
    println(io, df)
...
file = File{format"JLD2"}("Stats_precision.jld2")
